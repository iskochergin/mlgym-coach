...
def test_coach():
    # Mock task
    task = Task(id="test", description="desc", metric="auc", metric_higher_better=True, train_path="", test_features_path="")
    ...
    # 3. Тест смены стадии
    history.append(Step(idx=2, stage=Stage.EDA, action=Action(type=ActionType.CODE, content="df['target'].value_counts()"), result="ok"))
