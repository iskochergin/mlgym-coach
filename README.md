...
mlgym-coach/
  ...
  coach/       # [Человек 1] чек-лист, детекторы, подсказки L1–L3, грейдер, метрики
  dashboard/   # [Человек 2] streamlit-вьюер прогонов поверх EpisodeResult.json
  tasks/       # [Человек 3] датасеты + спеки задач (train + спрятанный test + метрика)
  runner/      # [Человек 3] харнесс экспериментов (фиксирует LLM+датасет, N сидов)
  reports/     # [Человек 3] агрегированные результаты, таблицы baseline vs scaffold
```
...
**Человек 3 — Runner + датасеты** (`tasks/`, `runner/`, `reports/`)
